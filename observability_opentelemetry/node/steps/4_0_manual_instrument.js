...
const provider = new NodeTracerProvider({
    resource: new Resource({
        [SemanticResourceAttributes.SERVICE_NAME]: 'aco-nodejs-manual-instru',
    }),
});
...
app.get('/', async (req, res) => {
    ...
    // propagation.inject({ getValue: () => parentSpan }, headers);

    // Make a request to the sub path
    try {
        await axios.get('http://localhost:3000/sub_action', { headers });
